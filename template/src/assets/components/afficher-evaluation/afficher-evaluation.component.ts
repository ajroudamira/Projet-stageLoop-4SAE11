...
export class AfficherEvaluationComponent implements OnInit {
  ...
  filterEvaluations(): void {
    this.filteredEvaluations = this.evaluations.filter(evaluation => 
      evaluation.studentName.toLowerCase().includes(this.searchQuery.toLowerCase()) ||
      evaluation.score.toString().includes(this.searchQuery) ||
      evaluation.comments.toLowerCase().includes(this.searchQuery.toLowerCase())
    );
  }
